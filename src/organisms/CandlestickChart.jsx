...
const CandlestickChart = ({ pair, data = [], height = 400 }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i <= numVerticalLines; i++) {
      ...
      if (i < numVerticalLines) {
        ...
        const time = new Date(data[dataIndex].timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
        ctx.fillStyle = "#9CA3AF" // Label color
        ctx.font = "10px Arial"
    ...
    data.forEach((candle, i) => {
      ...
      ctx.fillRect(x + xScale * 0.2, Math.min(open, close), xScale * 0.6, Math.abs(close - open))
